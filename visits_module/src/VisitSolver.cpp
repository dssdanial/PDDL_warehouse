...
void VisitSolver::loadSolver(string *parameters, int n)
{
    ...
    string waypoint_file = "./assignment_files/visits_domain/waypoint.txt";
    parseWaypoint(waypoint_file);

    string landmark_file = "./assignment_files/visits_domain/landmark.txt";
...
double VisitSolver::distance_euc(string from, string to)
{
   ...
   double distance2 = 0;
   for(int i=0; i<2; ++i)
        distance2 += pow(from_wp[i] - to_wp[i], 2);
